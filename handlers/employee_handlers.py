...
async def view_employees_list(message: types.Message):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get("http://localhost:8000/employee_table/tables/") as response:
                if response.status == 200:
                    employees = await response.json()
                    if employees:
                        text = "👥 Сотрудники:\n\n"
                        for emp in employees:
                            text += (f"👨🏻‍💼 ФИО: {emp.get('full_name', 'No name')}\n"
...
async def view_spec_endpoint_employee(message: types.Message):
    """Асинхронная функция для получения приоритетных исполнителей"""
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get("http://localhost:8000/employee_table/tables/find-employee/") as response:
                if response.status == 200:
                    employees = await response.json()
                    if employees:
                        text = "👥 Сотрудники:\n\n"
